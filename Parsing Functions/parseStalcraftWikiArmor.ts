...
import {
    ...
    GetAndCopyIcons, MinimizeItemInfo, MinimizeStalcraftWikiArmorInfo,
...
export const ParseStalcraftWikiArmor = async function ParseArmor(pathToItemsFolder = ''): Promise<object[]> {

    if (pathToItemsFolder === '' || !fs.existsSync(pathToItemsFolder)) {
        throw new Error('ParseStalcraftWikiArmor: incorrect or null path to folder');
    }
    ...
    fs.writeFileSync(resultFolder + '\\' + server + '_armors.json', JSON.stringify(MinimizeStalcraftWikiArmorInfo(SortByGearRanksAndCondition(AllArmors), server)));
