...
import {
    ...
    FindValueByKey, GetAndCopyIcons, MinimizeItemInfo, MinimizeStalcraftWikiContainerInfo,
    SortByGearRanks,
    SortProperties
...
export const ParseStalcraftWikiContainer = async function ParseContainer(pathToItemsFolder = ''): Promise<object[]> {
    ...
    parseItemsInFolder(pathToItemsFolder).then(() => {
        const CategoryPath = resultFolder + '\\' + server + `_containers.json`;
        fs.writeFileSync(CategoryPath, JSON.stringify(MinimizeStalcraftWikiContainerInfo(SortByGearRanks(AllContainers), server)));
